...
const ExpenseDetails = (props)=>{
  ...
  const clickHandler2 = ()=>{
    ...
    <div className="expense-item2"><div>{title}</div>
    <button onClick={clickHandler1} className='button2'>update title</button></div>
        <div className='expense-amount'>
      <div className="expense-price">${amount}</div></div>
      <button onClick={clickHandler2} className='button1'>update amount</button>
